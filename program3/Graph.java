 ...
 * @param hash 
    ...
    public void addVertex(String v, Map<String, MSUclass> hash) {
        if (!hasVertex(v)) st.put(v, new SET<String>());
        for (String i : st) { //Checks to see if the ID in our hash table's professor matches up with any
        	if(hash.get(i).getProf().equals(hash.get(v).getProf()) && (i!= v)) { //profs in our graph
        		addEdge(v, i);												//and if so then adds an edge
        	}	
   ...
   /**
     ...
     */
    public String toString(Map<String, MSUclass> hash) {
        StringBuilder s = new StringBuilder();
        System.out.println(this.V() + " Vertices, " + E() + " Edges");
        for (String v : st) {
        	if(st.get(v).isEmpty()) {
        		s.append(v + ": " + hash.get(v).getProf() + " teaches only this course");
        	}
        	else{
        		s.append(v + ": " + hash.get(v).getProf() + " also teaches ");
